...
from dataloader_data2 import NIH_Dataset, valid_data_transform, valid_data_transform_vit
...
def main():
    ...
    for data, mask, label in tqdm(testloader):
        ...
        plt.imsave(save_dir+"/cam/" + str(i) + ".png", visualization)
...
if __name__ == '__main__':
    ...
    Pre = 1
    type = 0
    size, intype, modelname, schedular, epochs = Hyperparameter(size=3, Pre=Pre, model=10, lrs=lrs, type=type)
    save_dir = 'D:/ExpResult/train/' + size + intype + modelname + schedular
    os.makedirs(save_dir + "/cam")
    residual_test = 0
    if residual_test == 0:
        test = "/test"
    ...
    test_bacteria_mask_path = 'D:/Datasets/'+size+test+'/bacteria/masks/'
    test_normal_mask_path = 'D:/Datasets/'+size+test+'/normal/masks/'
    test_virus_mask_path = 'D:/Datasets/'+size+test+'/virus/masks/'
    ...
    test_mask_paths = [[test_bacteria_mask_path + file for file in os.listdir(test_bacteria_mask_path)]
                      # + [test_normal_mask_path + file for file in os.listdir(test_normal_mask_path)]
                      + [test_virus_mask_path + file for file in os.listdir(test_virus_mask_path)]
