...
if __name__ == '__main__':
    ...
    type = 0
    test = '/test'
    size, intype, modelname, schedular, epochs = Hyperparameter(size=2, Pre=Pre, model=17, lrs=lrs, type=type)
    save_dir = 'D:/ExpResult/train/' + size + intype + modelname + schedular
    os.makedirs(save_dir)
    tb_writer = SummaryWriter(log_dir="D:/ExpResult/train/tensorboard" + modelname)
    ...
    trainloader = DataLoader(trainset, batch_size=24, shuffle=True, num_workers=4, pin_memory=False)
    ...
    if modelname == '/Efficient-Vit-T_Pretrain':
        ...
        model = our_model_2(num_classes=4)
    ...
    if Pre == 1:
        ...
        #optimizer = optim.SGD(pg, lr=0.0005, momentum=0.9, weight_decay=5e-4)
    if lrs == 1:
        scheduler = create_lr_scheduler(optimizer, len(trainloader), epochs,
                                       warmup=True, warmup_epochs=60, end_factor=1e-3)
