...
from dataloader_data2 import NIH_Dataset, train_data_transform, valid_data_transform, valid_data_transform_vit
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            if min(vloss1) > epoch_val_loss:
                ...
                estop += 1
                if lrs == 0:
                    print("Counter {} of xx".format(estop))
                    # if estop > 22 and epoch>30:
...
if __name__ == '__main__':
    ...
    type = 0
    test = '/test'
    size, intype, modelname, schedular, epochs = Hyperparameter(size=3, Pre=Pre, model=8, lrs=lrs, type=type)
    save_dir = 'D:/ExpResult/train/' + size + intype + modelname + schedular
    os.makedirs(save_dir)
    tb_writer = SummaryWriter(log_dir="D:/ExpResult/train/tensorboard1" + modelname)
    ...
    train_virus_image_path = 'D:/Datasets/'+size+'/train/virus/images/'
    # Paths to images
    val_bacteria_image_path = 'D:/Datasets/'+size+test+'/bacteria/images/'
    val_normal_image_path = 'D:/Datasets/'+size+test+'/normal/images/'
    val_virus_image_path = 'D:/Datasets/'+size+test+'/virus/images/'


    # Paths to masks
    train_bacteria_mask_path = 'D:/Datasets/'+size+'/train/bacteria/masks/'
    train_normal_mask_path = 'D:/Datasets/'+size+'/train/normal/masks/'
    train_virus_mask_path = 'D:/Datasets/'+size+'/train/virus/masks/'
    # Paths to masks
    val_bacteria_mask_path = 'D:/Datasets/'+size+test+'/bacteria/masks/'
    val_normal_mask_path = 'D:/Datasets/'+size+test+'/normal/masks/'
    val_virus_mask_path = 'D:/Datasets/'+size+test+'/virus/masks/'


    # All paths to images and masks
    train_image_paths = [[train_bacteria_image_path + file for file in os.listdir(train_bacteria_image_path)]
                         + [train_normal_image_path + file for file in os.listdir(train_normal_image_path)]
                         + [train_virus_image_path + file for file in os.listdir(train_virus_image_path)]
                         ][0]
    train_mask_paths = [[train_bacteria_mask_path + file for file in os.listdir(train_bacteria_mask_path)]
                        + [train_normal_mask_path + file for file in os.listdir(train_normal_mask_path)]
                        + [train_virus_mask_path + file for file in os.listdir(train_virus_mask_path)]
                        ][0]
    val_image_paths = [[val_bacteria_image_path + file for file in os.listdir(val_bacteria_image_path)]
                       + [val_normal_image_path + file for file in os.listdir(val_normal_image_path)]
                       + [val_virus_image_path + file for file in os.listdir(val_virus_image_path)]
                       ][0]
    val_mask_paths = [[val_bacteria_mask_path + file for file in os.listdir(val_bacteria_mask_path)]
                      + [val_normal_mask_path + file for file in os.listdir(val_normal_mask_path)]
                      + [val_virus_mask_path + file for file in os.listdir(val_virus_mask_path)]
                      ][0]

    trainset = NIH_Dataset(train_image_paths, train_mask_paths, transform=train_data_transform, train=1, type=type)
    trainloader = DataLoader(trainset, batch_size=64, shuffle=True, num_workers=2, pin_memory=False)
    ...
    if modelname == '/Proposed_Pretrain'or modelname == '/Proposed_2_Pretrain':
        model = our_model(num_classes=3)
    ...
    if modelname == '/Efficient-Vit-T_Pretrain':
        model = efficient_vit_T(num_classes=3)
        pretrained_dict = torch.load(r'D:\ExpResult\pretrain_parameters\efficientnetV2s-imagenet.pth')
        net_state_dict = model.state_dict()
        ...
        model = resnext50_32x4d1(num_classes=3)
        pretrained_dict = torch.load(r'D:\ExpResult\pretrain_parameters\resneXt50-imagenet.pth')
        net_state_dict = model.state_dict()
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if not k.startswith('fc') if k in net_state_dict}
    elif modelname == '/Vit-16_Pretrain':
        model = vit_tiny_patch16_224(num_classes=3)
        #pretrained_dict = torch.load(r'D:\ExpResult\pretrain_parameters\vit_base-imagenet.pth')
        pretrained_dict = torch.load(r'D:\ExpResult\pretrain_parameters\NIH_2vit16.pkl')
        net_state_dict = model.state_dict()
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if not k.startswith('head') if k in net_state_dict}
        ...
        model = vit_base_patch32(num_classes=3)
        ...
        model = res18_vit_T(num_classes=3)
    ...
    if lrs == 1:
        scheduler = create_lr_scheduler(optimizer, len(trainloader), epochs,
                                       warmup=True, warmup_epochs=60, end_factor=1e-5)
